/**
 * Additional state definition by implementing the default metaplex states
 *
 * By the way, make accounts explicity in the contexts of all Ixs as much as possible
 * This will helpful to make instruction accounts as Boxed account
 * and remove unnecessary /// CHECK: definition against unsafe AccountInfo usages
 */
...
#[derive(Clone, AnchorDeserialize, AnchorSerialize)]
pub struct TokenRecord(mpl_token_metadata::state::TokenRecord);

impl anchor_lang::AccountDeserialize for TokenRecord {
    fn try_deserialize_unchecked(buf: &mut &[u8]) -> Result<Self> {
        try_from_slice_checked::<mpl_token_metadata::state::TokenRecord>(
            buf,
            mpl_token_metadata::state::Key::MetadataV1,
            MAX_METADATA_LEN,
        )
